...
export default function AdminDashboardClient({ metrics, blogs, pendingCreators = [] }) {
    ...
    const handleApprove = async (blogId) => {
        ...
        try {
            await fetch(`/api/blogs/${blogId}`, {
                ...
                body: JSON.stringify({ action: "moderate", status: "approved" }),
            });
            router.refresh();
        } catch (e) {
    ...
    const handleRejectSubmit = async () => {
        ...
        try {
            await fetch(`/api/blogs/${selectedBlog._id}`, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ action: "moderate", status: "rejected", rejectionReason }),
            });
            if (revokeCreator) {
                await fetch(`/api/admin/users/${selectedBlog.authorId}/role`, {
                    method: "PUT",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ action: "revoke" }),
                });
                alert(`Blog rejected and creator privileges permanently revoked for ${selectedBlog.authorName}.`);
            }
    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-black text-gray-900 dark:text-gray-100 transition-colors pb-20">
            ...
            <div className="max-w-7xl mx-auto px-4 md:px-8">
                ...
                <div className="flex space-x-2 mb-6 p-1 bg-gray-200 dark:bg-gray-900 rounded-xl w-fit overflow-x-auto">
                    {[
                        ...
                        <button
                            ...
                            className={`px-6 py-2 rounded-lg font-bold text-sm tracking-wide capitalize transition-all whitespace-nowrap ${activeTab === tab.id
                ...
                <div className="space-y-4">
                    {activeTab === "apps" && pendingCreators.length === 0 && (
                    ...
                    {activeTab === "apps" && pendingCreators.map((user) => (
