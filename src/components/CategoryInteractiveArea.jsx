...
export default function CategoryInteractiveArea({ category, initialArticles = [], userFirstName = "Guest" }) {
            ...
            {filtered.length > 0 ? (
                <>
                    {searchQuery && (
                        <p className="mb-8 text-gray-500 dark:text-gray-400 font-medium italic">
                            Showing {filtered.length} results for "{searchQuery}" in {category}
                        </p>
                    )}
                ...
                <div className="text-center py-20 bg-white dark:bg-gray-900 rounded-3xl border-2 border-dashed border-gray-100 dark:border-gray-800">
                    ...
                    <p className="text-gray-500 dark:text-gray-400 text-xl font-bold mb-2">
                        {searchQuery ? `No matches found for "${searchQuery}" in ${category}` : "No blogs found"}
                    </p>
                    {searchQuery && (
                        <button
