...
export default function CategoryPage({ params: paramsPromise }) {
  ...
  useEffect(() => {
    const CACHE_KEY = `category_cache_${category}`;
      ...
      try {
        ...
        if (!force && cachedData) {
          ...
          if (now - timestamp < ONE_HOUR) {
            console.log(`Using cached ${category} news`);
      ...
      {loading ? (
        ...
        <>
          {searchQuery && (
            <p className="mb-8 text-gray-500 dark:text-gray-400 font-medium italic">
              Showing {filteredArticles.length} results for "{searchQuery}" in {category}
            </p>
          )}
          <div className="columns-1 md:columns-2 lg:columns-3 gap-6 space-y-6 md:space-y-0">
            {filteredArticles.map((article, index) => (
              <div key={index} className="break-inside-avoid mb-6">
                <BlogCard blog={article} category={category} />
              </div>
            ))}
          </div>
        </>
        ...
        <div className="text-center py-20 bg-white dark:bg-gray-900 rounded-3xl border-2 border-dashed border-gray-100 dark:border-gray-800 shadow-sm transition-colors duration-300">
          ...
          <p className="text-gray-500 dark:text-gray-400 text-xl font-bold mb-2">
            {searchQuery ? `No matches found for "${searchQuery}" in ${category}` : "No blogs found in this category"}
          </p>
